...
<div class="pt-10 ps-40">
    ...
    <div class="flex justify-center gap-20 border-2 py-2 mt-6 mr-4 rounded-md">
        <div>
            <a href="/status-orderan" class="{{ request()->url() === url('/status-orderan') && !request('status') ? 'text-blue1 font-bold border-gray-400 border-b-2 px-2' : 'text-gray-400' }}">Semua</a>
        </div>
        @foreach ($status as $item)
        <div class="flex justify-center">
            <div>
                <a href="/status-orderan?status={{ $item }}" class="{{ request('status') === $item ? ' text-blue1 font-bold border-gray-400 border-b-2 px-2' : 'text-gray-400' }}">{{ ucwords(str_replace('-', ' ', $item)) }}</a>
            </div>
        </div>   
        @endforeach
            ...
            @foreach ($transaksis as $penjual => $transaksiListByTime)
                @foreach ($transaksiListByTime as $createdAt => $transaksiList)
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-3">
                            <i class="fa-solid fa-circle-check text-green-600 mr-3"></i>
