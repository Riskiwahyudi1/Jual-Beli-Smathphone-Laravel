    ...
    <tbody>
                ...
                @foreach ($transaksis as $penjual => $transaksiListByTime)
                    @foreach ($transaksiListByTime as $createdAt => $transaksiList)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            @if (request()->query('status') == 'menunggu-pembayaran')
                                ...
                                @else
                                    <td class="px-6 py-3">
                                        <button data-modal-target="bukti-transaksi-file{{ $transaksiList->first()->id}}" data-modal-toggle="bukti-transaksi-file{{ $transaksiList->first()->id}}" class="text-blue2"><i class="fa-solid fa-eye mr-1"></i>Pembayaran</button>
                                    </td>
                                @endif 
                            @endif
                            <td class="px-6 py-3">
                        ...
                        <div id="bukti-transaksi-file{{ $transaksiList->first()->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4  max-w-lg max-h-lg">
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <img class="h-11/12 w-11/12 mr-3" src="{{ asset($transaksiList->first()->bukti_pembayaran) }}" alt="image description">
                    ...
                    <div class=" text-red-600">
                        <p class="font-semibold">Transaksi "<span class="text-black">{{ request('search') }}</span>" tidak ditemukan!</p>
                    </div>
                    @elseif (request('status'))
                    <div class=" text-red-600">
                        <p class="font-semibold">Belum ada transaksi yang {{ request('status') }}</p>
                    </div>
                @else
                    <div class=" text-red-600">
                        <p class="font-semibold">Belum ada transaksi di toko anda!</p>
                    </div>
                @endif
            </tbody>
        </table>
